...
| S7 | Full-chain T0 latency: 40-object app, 300-CRD snapshot, 50 Kyverno policies, warm; Kyverno CLI cold start measured separately | Chain under 2 s or the copy changes; Kyverno stage moves to save-time if over budget | interrupted 2026-09-18 at the fixtures stage by the API spend limit; partial fixtures committed under `s7-chain-latency/testdata/` (CRDs, Kyverno policies, app charts); resume: `Workflow({scriptPath: workflows/scripts/s7-chain-latency-wf_60d6a816-277.js, resumeFromRunId: wf_60d6a816-277})` or rerun the spike by hand |
| S8 | Import size/time/RBAC degradation on EKS, AKS, OpenShift with discovery-only, namespace-scoped and full identities; MAP v1/v1beta1 fallback | Per-layer degradation recorded; namespace-scoped import is a supported mode | interrupted 2026-09-18 after the identities stage by the API spend limit (kind only, no EKS/AKS/OpenShift); RBAC identities and per-layer probe committed under `s8-import-degradation/testdata/` (kubeconfigs with tokens git-ignored); importer code half-written and not committed; resume: `resumeFromRunId: wf_99572b06-7a9` |
...
| S13 | Troubleshoot analyzers on a snapshot-as-bundle adapter | `storageClass`, `clusterVersion`, `customResourceDefinition`, `nodeResources` pass on a synthetic bundle | interrupted 2026-09-18 after the scout stage by the API spend limit; fixture bundle and analyzer specs committed under `s13-troubleshoot-analyzers/testdata/`; adapter code half-written and not committed; resume: `resumeFromRunId: wf_c9f86932-b67` |
