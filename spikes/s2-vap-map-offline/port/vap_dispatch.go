...
// Fathom edit: upstream file is pkg/admission/plugin/policy/validating/dispatcher.go
// plus reasonToCode from policy_decision.go. The dispatcher struct and its
// Dispatch method are exported; package-local types are qualified with
// "validating.". Nothing else changed.
...
// ValidatingDispatcher is validating.dispatcher (dispatcher.go:42-57).
type ValidatingDispatcher struct {
	matcher generic.PolicyMatcher
	authz   authorizer.UnconditionalAuthorizer
}

var _ generic.Dispatcher[validating.PolicyHook] = &ValidatingDispatcher{}

// NewValidatingDispatcher is validating.NewDispatcher (dispatcher.go:49-57).
func NewValidatingDispatcher(
...
func (c *ValidatingDispatcher) Start(ctx context.Context) error {
	return nil
}

// Dispatch implements generic.Dispatcher.
func (c *ValidatingDispatcher) Dispatch(ctx context.Context, a admission.Attributes, o admission.ObjectInterfaces, hooks []validating.PolicyHook) error {
	var deniedDecisions []policyDecisionWithMetadata
	var validationFailures []validating.ValidationFailureValue
	...
	addConfigError := func(err error, definition *admissionregistrationv1.ValidatingAdmissionPolicy, binding *admissionregistrationv1.ValidatingAdmissionPolicyBinding) {
		...
		case admissionregistrationv1.Fail:
			...
			deniedDecisions = append(deniedDecisions, policyDecisionWithMetadata{
				PolicyDecision: validating.PolicyDecision{
	...
	for _, hook := range hooks {
		...
		matches, matchResource, matchKind, err := c.matcher.DefinitionMatches(a, o, validating.NewValidatingAdmissionPolicyAccessor(definition))
		if err != nil {
			// Configuration error.
		...
		for _, binding := range hook.Bindings {
			...
			matches, err := c.matcher.BindingMatches(a, o, validating.NewValidatingAdmissionPolicyBindingAccessor(binding))
			if err != nil {
				// Configuration error.
			...
			for _, validationResult := range validationResults {
				for i, decision := range validationResult.Decisions {
					...
					case validating.ActionAdmit:
						if decision.Evaluation == validating.EvalError {
							celmetrics.Metrics.ObserveAdmission(ctx, decision.Elapsed, definition.Name, binding.Name, validating.ErrorType(&decision))
						}
					case validating.ActionDeny:
						for _, action := range binding.Spec.ValidationActions {
							...
							case admissionregistrationv1.Deny:
								...
								celmetrics.Metrics.ObserveRejection(ctx, decision.Elapsed, definition.Name, binding.Name, validating.ErrorType(&decision))
							case admissionregistrationv1.Audit:
								validationFailures = append(validationFailures, validating.ValidationFailureValue{
									ExpressionIndex:   i,
								...
								celmetrics.Metrics.ObserveAudit(ctx, decision.Elapsed, definition.Name, binding.Name, validating.ErrorType(&decision))
							case admissionregistrationv1.Warn:
								...
								celmetrics.Metrics.ObserveWarn(ctx, decision.Elapsed, definition.Name, binding.Name, validating.ErrorType(&decision))
							}
						}
					default:
				...
				for _, auditAnnotation := range validationResult.AuditAnnotations {
					...
					case validating.AuditAnnotationActionPublish:
					...
					case validating.AuditAnnotationActionError:
						...
						d := policyDecisionWithMetadata{
							...
							PolicyDecision: validating.PolicyDecision{
								Action:     validating.ActionDeny,
								Evaluation: validating.EvalError,
								Message:    auditAnnotation.Error,
								Elapsed:    auditAnnotation.Elapsed,
							},
						}
						deniedDecisions = append(deniedDecisions, d)
						celmetrics.Metrics.ObserveRejection(ctx, auditAnnotation.Elapsed, definition.Name, binding.Name, validating.ErrorType(&d.PolicyDecision))
					case validating.AuditAnnotationActionExclude: // skip it
					default:
...
func publishValidationFailureAnnotations(attributes admission.Attributes, failures []validating.ValidationFailureValue) {
	if len(failures) == 0 {
		return
	}
...
func formatPoliciesBindings(failures []validating.ValidationFailureValue) string {
...
// reasonToCode is validating.reasonToCode (policy_decision.go:73-87).
