...
// Fathom edit: upstream file is pkg/admission/plugin/policy/mutating/dispatcher.go.
// The struct is exported, Start (which spawned the TypeConverterManager poll
// loop) is dropped because the snapshot manager is synchronous, and
// package-local names are qualified with "mutating.". The delegate is handed
// to the exported generic.NewPolicyDispatcher exactly as upstream does, so the
// hook/binding matching and the failure-policy filter are not copied.
...
// NewMutatingDispatcher is mutating.NewDispatcher (dispatcher.go:46-59).
func NewMutatingDispatcher(a authorizer.UnconditionalAuthorizer, m *matching.Matcher, tcm patch.TypeConverterManager) generic.Dispatcher[mutating.PolicyHook] {
	res := &MutatingDispatcher{
		matcher:              m,
		authz:                a,
		typeConverterManager: tcm,
	}
	res.Dispatcher = generic.NewPolicyDispatcher[*mutating.Policy, *mutating.PolicyBinding, mutating.PolicyEvaluator](
...
// MutatingDispatcher is mutating.dispatcher (dispatcher.go:61-66).
type MutatingDispatcher struct {
	matcher              *matching.Matcher
	authz                authorizer.UnconditionalAuthorizer
	typeConverterManager patch.TypeConverterManager
	generic.Dispatcher[mutating.PolicyHook]
}

func (d *MutatingDispatcher) dispatchInvocations(
	ctx context.Context,
	a admission.Attributes,
	o admission.ObjectInterfaces,
	versionedAttributes webhookgeneric.VersionedAttributeAccessor,
	invocations []generic.PolicyInvocation[*mutating.Policy, *mutating.PolicyBinding, mutating.PolicyEvaluator],
	...
	if v := reinvokeCtx.Value(mutating.PluginName); v != nil {
		policyReinvokeCtx = v.(*policyReinvokeContext)
	} else {
		policyReinvokeCtx = &policyReinvokeContext{}
		reinvokeCtx.SetValue(mutating.PluginName, policyReinvokeCtx)
	}
	...
	addConfigError := func(err error, invocation generic.PolicyInvocation[*mutating.Policy, *mutating.PolicyBinding, mutating.PolicyEvaluator], reason metav1.StatusReason) {
		policyErrors = append(policyErrors, generic.PolicyError{
			Message: err,
			Policy:  mutating.NewMutatingAdmissionPolicyAccessor(invocation.Policy),
			Binding: mutating.NewMutatingAdmissionPolicyBindingAccessor(invocation.Binding),
			Reason:  reason,
		})
	}
	...
	for _, invocation := range invocations {
		...
		for mutationIndex := range invocation.Policy.Spec.Mutations {
			...
			if err != nil {
				celmetrics.Metrics.ObserveRejection(ctx, elapsed, invocation.Policy.Name, invocation.Binding.Name, mutating.ErrorType(err))
				var statusError *k8serrors.StatusError
				if errors.As(err, &statusError) {
					return nil, statusError
				}
...
func (d *MutatingDispatcher) dispatchOne(
	ctx context.Context,
...
func keyFor(invocation generic.PolicyInvocation[*mutating.Policy, *mutating.PolicyBinding, mutating.PolicyEvaluator]) (key, error) {
