...
// Fathom edit: upstream file is pkg/admission/plugin/policy/mutating/compilation.go.
// compilePolicy called environment.MustBaseEnvSet(environment.DefaultCompatibilityVersion())
// inline; here the caller passes the *environment.EnvSet.
...
// CompileMutatingPolicy is mutating.compilePolicy (compilation.go:36-82). It
// compiles the policy into a PolicyEvaluator; any error is stored and delayed
// until invocation.
//
...
func CompileMutatingPolicy(policy *v1.MutatingAdmissionPolicy, envSet *environment.EnvSet) mutating.PolicyEvaluator {
