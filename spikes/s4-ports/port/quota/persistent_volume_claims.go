...
// fathom port of k8s.io/kubernetes/pkg/quota/v1/evaluator/core/persistent_volume_claims.go
// at tag v1.37.0. Changes against upstream, each marked "fathom:" below:
//   - the *api.PersistentVolumeClaim arm of the converter is dropped;
//   - helper.ScopedResourceSelectorRequirementsAsSelector is the copy in helpers.go;
//   - VolumeAttributesClass and RecoverVolumeExpansionFailure are GA and locked
//     on at 1.37 (port/gates); the gate reads stay as constants and the disabled
//     else branches are deleted.
// Everything else is upstream text in upstream order.
...
func (p *pvcEvaluator) Matches(resourceQuota *corev1.ResourceQuota, item runtime.Object) (bool, error) {
	if gates.VolumeAttributesClass {
...
func (p *pvcEvaluator) getStorageUsage(pvc *corev1.PersistentVolumeClaim) *resource.Quantity {
	...
	if gates.RecoverVolumeExpansionFailure && result != nil {
...
// fathom: the *api.PersistentVolumeClaim conversion arm is gone.
...
func RequiresQuotaReplenish(pvc, oldPVC *corev1.PersistentVolumeClaim) bool {
	if gates.RecoverVolumeExpansionFailure {
