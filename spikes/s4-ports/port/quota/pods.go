...
// fathom port of k8s.io/kubernetes/pkg/quota/v1/evaluator/core/pods.go at tag
// v1.37.0. Changes against upstream, each marked "fathom:" below:
//   - the *api.Pod arm of toExternalPodOrError and its converter import are
//     dropped (fathom only ever holds *corev1.Pod);
//   - helper.IsNativeResource, helper.ScopedResourceSelectorRequirementsAsSelector,
//     qos.GetPodQOS and schedutil.IsDRAExtendedResourceName come from helpers.go
//     and qos.go in this package (inlined, they are k8s.io/kubernetes-only);
//   - feature gates are the constants in port/gates.
// Everything else is upstream text in upstream order; quota.* and generic.* are
// the unchanged staging packages.
...
func isExtendedResourceNameForQuota(name corev1.ResourceName) bool {
	...
	nonNative := !IsNativeResource(name)
...
func (p *podEvaluator) Constraints(required []corev1.ResourceName, item runtime.Object) error {
	...
	// at pod level.
	if gates.PodLevelResources && resourcehelper.IsPodLevelResourcesSet(pod) {
		return nil
	}
...
func podComputeUsageHelper(requests corev1.ResourceList, limits corev1.ResourceList) corev1.ResourceList {
	...
	for resource, request := range requests {
		...
		// for extended resources
		if IsDRAExtendedResourceName(resource) {
...
// fathom: the *api.Pod conversion arm is gone; only external pods exist here.
...
func PodUsageFunc(obj runtime.Object, clock clock.Clock) (corev1.ResourceList, error) {
	...
	opts := resourcehelper.PodResourcesOptions{
		UseStatusResources: gates.InPlacePodVerticalScaling,
		// SkipPodLevelResources is set to false when PodLevelResources feature is enabled.
		SkipPodLevelResources: !gates.PodLevelResources,
	}
